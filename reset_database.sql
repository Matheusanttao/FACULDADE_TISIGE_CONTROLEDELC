-- ============================================================================
-- TISIGE — RESET COMPLETO DO SCHEMA PUBLIC (dados + estrutura da app)
-- ============================================================================
--
-- O QUE FAZ:
--   • Apaga TODO o schema public (tabelas, funções, triggers, policies, etc.)
--   • Recria o banco igual às migrations do projeto (ordem correta)
--
-- O QUE NÃO APAGA:
--   • auth.users (utilizadores do Supabase Authentication) — ficam no painel
--   • Storage, Edge Functions, etc.
--
-- Depois do reset:
--   • Cada utilizador em Authentication continua a existir, mas pode não ter
--     linha em public.profiles até o trigger correr de novo (novo registo) ou
--     inserires manualmente. Se quiseres lista limpa: Authentication → Users
--     → apagar utilizadores à mão.
--
-- Como executar:
--   • Supabase Dashboard → SQL Editor → colar este ficheiro → Run
--   • Ou: psql / supabase db execute (role postgres)
--
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1) Destruir e recriar o schema public
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2) Extensão (gen_random_uuid, etc.)
-- ---------------------------------------------------------------------------
create extension if not exists pgcrypto;

-- ---------------------------------------------------------------------------
-- 3) Migration: 20260324000000_initial_schema.sql
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4) Migration: 20260325120000_approval_flow.sql
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5) Migration: 20260326100000_admin_profiles_policy.sql
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 6) Permissões Supabase (tabelas/funções já criadas)
-- ---------------------------------------------------------------------------
...
-- ============================================================================
-- Fim. Opcional: voltar a criar perfis para utilizadores Auth existentes
-- (só se precisares — o trigger só corre em NOVOS registos em auth.users)
-- ============================================================================
-- insert into public.profiles (id, username, email, full_name, tipo, papel)
-- select u.id, split_part(u.email,'@',1), u.email, '', 'B', 'desenhista'
-- from auth.users u
